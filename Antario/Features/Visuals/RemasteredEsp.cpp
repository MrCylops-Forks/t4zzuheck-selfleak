...
void RemasteredEsp::Render()
{
	...
	for (int i = 1; i < g_pEngine->GetMaxClients(); i++)
	{
		...
		/*
			Distance
			float dd = sqrtf((PosX - plr[0]) * (PosX - plr[0]) + (PosY - plr[1]) * (PosY - plr[1]) + (PosZ - plr[2]) * (PosZ - plr[2]));
			dd *= 0.01905f;
		*/
		...
		// Health Line
		int EntHealth = pEnt->GetHealth();
		int HealthPixelValue = EntHealth * EspWidth / 100;

		g_pSurface->RoundedFilledRect(ScreenHeadPosition.x - (EspWidth / 2), ScreenHeadPosition.y - 6, EspWidth, 3, 2, Color(33, 33, 33, 100));
		g_pSurface->RoundedFilledRect(ScreenHeadPosition.x - (EspWidth / 2), ScreenHeadPosition.y - 6, HealthPixelValue, 3, 2, g_Misc.getHealthColor(pEnt));
