...
{
	g_pSurface->OutlinedRect(Box.left + 1, Box.top + 1, Box.right - 2, Box.bottom - 2, shadow);
	g_pSurface->OutlinedRect(Box.left - 1, Box.top - 1, Box.right + 2, Box.bottom + 2, shadow);
...
//	const int fade = ( int )( ( ( 1 / 0.25f ) * interfaces::globals->frame_time ) * 255 );
	...
	Vector position;
	if (WorldToScreen(GetHitboxPosition(player, 2), position) && !g_Menu.Config.FovAlways)
		return;
...
void ESP::RenderName(C_BaseEntity* pEnt, int iterator)
{
	...
	if (MultiByteToWideChar(CP_UTF8, 0, pInfo.szName, -1, buffer, 36) > 0)
	{
		...
		{
			g_pSurface->DrawT(Box.left, Box.top - 12, namecolor, g::EspTahoma, false, name);
		}
		else if (g_Menu.Config.Name == 2)
		{
			g_pSurface->DrawT(Box.left + (Box.right / 2), Box.top - 12, namecolor, g::EspTahoma, true, name);
		}
	}
}
...
void ESP::RenderWeaponName(C_BaseEntity* pEnt)
{
	...
	int maxclip = weapon->GetCSWpnData()->max_clip;
	...
	else if (g_Menu.Config.Weapon == 2)
	{
		...
		g_pSurface->DrawT(Box.left + (Box.right / 2), (Box.top + Box.bottom) + 1, weaponcolor, g::WeaponIcons, true, buffer);
		if (!pEnt->IsKnifeorNade())
...
void ESP::RenderEntHealth(C_BaseEntity* pEnt)
{
	...
	{
		g_pSurface->FilledRect(Box.left - 5, Box.top - 1, 3, Box.bottom + 2, shadow);
		int pixelValue = pEnt->GetHealth() * Box.bottom / 100;
		g_pSurface->FilledRect(Box.left - 4, Box.top + Box.bottom - pixelValue, 1, pixelValue, hbcolor);
	}
...
void ESP::Render()
{
	...
	/*float dmg = 0.f;
	if (g::pLocalEntity->IsAlive() && g_Menu.Config.AutoWallCrosshair)
	{
		matrix3x4_t matrix = g_Aimbot.Matrix(g::pLocalEntity->EntIndex());
		if (g_Autowall.CanHitFloatingPoint(g::pLocalEntity->GetHitboxPosition(HITBOX_HEAD, ), g::pLocalEntity->GetEyePosition()))
			g_pSurface->Line(center_width - 1, center_height, center_width + 1, center_height, Color(0, 255, 0, 255));
		else
			g_pSurface->Line(center_width - 1, center_height, center_width + 1, center_height, Color(255, 0, 0, 255));
	}*/
