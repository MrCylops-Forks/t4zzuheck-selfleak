...
void Resolver::AnimationFix(C_BaseEntity* pEnt)
{
	...
	if (storedSimtime[pEnt->EntIndex()] != pEnt->GetSimulationTime())
	{
		...
		if (Resim > g_AntiAim.MaxDelta(pEnt))
			Resim = g_AntiAim.MaxDelta(pEnt);
		if (Resim < -g_AntiAim.MaxDelta(pEnt))
			Resim = -g_AntiAim.MaxDelta(pEnt);
		...
		if (pEnt->GetVelocity().Length2D() > 0.5f && !shot)
		{
			...
			{
				Resim += (g_AntiAim.MaxDelta(pEnt) - Resim);
				DesyncFix += (g_AntiAim.MaxDelta(pEnt) - Resim);
			}
		...
		else if (!shot)
		{
			...
			if (Delta > g_AntiAim.MaxDelta(pEnt))
				Delta = g_AntiAim.MaxDelta(pEnt);
			if (Delta < -g_AntiAim.MaxDelta(pEnt))
				Delta = -g_AntiAim.MaxDelta(pEnt);
